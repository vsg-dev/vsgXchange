...
vsg::ref_ptr<vsg::Object> ReaderWriter_glsl::readFile(const vsg::Path& filename) const
{
...
bool ReaderWriter_glsl::writeFile(const vsg::Object* object, const vsg::Path& filename) const
{
    auto ext = vsg::fileExtension(filename);
    auto stage_itr = extensionToStage.find(ext);
